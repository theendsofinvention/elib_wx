...
from .structs import (Cloud, MetarData, MetarTrans, Number, TafData, TafLineData, TafLineTrans, TafTrans, Units)

LOGGER = logging.getLogger('elib.wx')


def get_cardinal_direction(direction: float) -> str:  # noqa
    """
...
def wind(direction: Number,
    ...
    # Wind direction
    if direction:
        if direction.repr is not None and direction.repr in WIND_DIR_REPR:
            ret += WIND_DIR_REPR[direction.repr]
        elif direction.value is None and direction.repr is not None:
            ret += direction.repr
        else:
            if cardinals and direction.value is not None:
...
def visibility(vis: Number, unit: str = 'm') -> str:
    ...
    Ex: 8km ( 5sm )
    """
    if vis is None or unit not in ('m', 'sm'):
        return ''
    ...
    if unit == 'm':
        ...
        converted_str = str(round(converted, 1)).replace('.0', '') + 'sm'
        value = str(round(vis.value / 1000, 1)).replace('.0', '')
        unit = 'km'
    elif unit == 'sm':
        converted = vis.value / 0.621371
        converted_str = str(round(converted, 1)).replace('.0', '') + 'km'
        value = str(vis.value).replace('.0', '')
    return f'{value}{unit} ({converted_str})'


def temperature(temp: typing.Optional[Number], unit: str = 'C') -> str:
    """
    ...
    """
    unit = unit.upper()
    if temp is None or temp.value is None or unit not in ('C', 'F'):
        return ''
    if unit == 'C':
        converted = temp.value * 1.8 + 32
        converted_str = str(int(round(converted))) + '°F'
    elif unit == 'F':
        converted = (temp.value - 32) / 1.8
        converted_str = str(int(round(converted))) + '°C'
    return f'{temp.value}°{unit} ({converted_str})'
...
def altimeter(alt: Number, unit: str = 'hPa') -> str:
    ...
    """
    if alt == '' or alt is None or alt.value is None or alt.repr is None or unit not in ('hPa', 'inHg'):
        return ''
    if unit == 'hPa':
        value = alt.repr
        converted = alt.value / 33.8638866667
        converted_str = str(round(converted, 2)) + ' inHg'
    elif unit == 'inHg':
        value = alt.repr[:2] + '.' + alt.repr[2:]
        converted = float(value) * 33.8638866667
        converted_str = str(int(round(converted))) + ' hPa'
    return f'{value} {unit} ({converted_str})'


def clouds(clds: typing.List[Cloud], unit: str = 'ft') -> str:
    """
...
def other_list(wxcodes: typing.List[str]) -> str:
    """
...
def turb_ice(turbice: typing.List[str], unit: str = 'ft') -> str:
    ...
    for i in reversed(range(len(split) - 1)):
        _si2_9 = split[i][2] == '9'
        _si0_n = split[i][0] == split[i + 1][0]
        # noinspection PyTypeChecker
        if _si2_9 and _si0_n and int(split[i + 1][1]) == (int(split[i][1]) + int(split[i][2]) * 10):
            # noinspection PyTypeChecker
...
def min_max_temp(temp: typing.Optional[str], unit: str = 'C') -> str:
    ...
    if temp[:2] == 'TX':
        ...
        return ''
    temp_list: list = temp[2:].replace('M', '-').replace('Z', '').split('/')
    if len(temp_list[1]) > 2:
        temp_list[1] = temp_list[1][:2] + '-' + temp_list[1][2:]
    temp_value = temperature(core.make_number(temp_list[0]), unit)
    return f'{temp_type} temperature of {temp_value} at {temp_list[1]}:00Z'


def shared(wxdata: typing.Union[MetarData, TafLineData], units: Units) -> typing.Dict[str, typing.Any]:
    """
    Translate Visibility, Altimeter, Clouds, and Other
    """
    translations = {
        'visibility': visibility(wxdata.visibility, units.visibility),
        'altimeter': altimeter(wxdata.altimeter, units.altimeter), 'clouds': clouds(wxdata.clouds, units.altitude),
        'other': other_list(wxdata.other)
    }
    return translations
...
def taf(wxdata: TafData, units: Units) -> TafTrans:
    ...
    """
    translations: typing.Dict[str, typing.Any] = {'forecast': []}
    ...
    if wxdata.min_temp:
        _min_temp: typing.Optional[str] = str(wxdata.min_temp)
    else:
        _min_temp = None
    if wxdata.max_temp:
        _max_temp: typing.Optional[str] = str(wxdata.max_temp)
    else:
        _max_temp = None
    translations['min_temp'] = min_max_temp(_min_temp, units.temperature)
    translations['max_temp'] = min_max_temp(_max_temp, units.temperature)
    translations['remarks'] = remarks.translate(wxdata.remarks)
